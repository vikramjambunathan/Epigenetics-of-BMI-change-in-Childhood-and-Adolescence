...
Time points remaining: F9,F11,F12,F13,F14,F15,F18,F24
...
bmi9 <- bmi_data %>% filter(time == "F9") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi9 <- bmi9[order(bmi9$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi9[c(3:24)]
cpg_assocF9 <- cpg.assoc(betas_cord, bmi9$bmi, covariates = covar)
cpg_assocF9
...
cpg_assoc_results = cpg_assocF9$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF9 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assoc,cpg_locations$TargetID,cpg_locations$CHR,cpg_locations$MAPINFO)

```
```{r}
bmi_data_linked$sex <- gsub("-9999","NA",as.character(bmi_data_linked$sex))
bmi_data_linked$sex<- gsub("1","Male",as.character(bmi_data_linked$sex))
bmi_data_linked$sex<- gsub("2","Female",as.character(bmi_data_linked$sex))
bmi_data_linked$mumsmoke <- gsub("-9999","NA",as.character(bmi_data_linked$mumsmoke))
bmi_data_linked %>%
  filter(time == "F9"& mumsmoke!= -1) %>%
