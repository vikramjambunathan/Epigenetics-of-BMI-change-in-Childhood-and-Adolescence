...
# Considering bmi at 14 years.

bmi14 <- bmi_data %>% filter(time == "F14") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()


```


1. Remove columns of betas_cord for Sample Names not in bmi14
...
5. Run cpg.assoc(betas_cord, bmi14$bmi)
6. Table of top 10 cpg
7. Manhattan plot (Annotate)


```{r}
# For bmi14, we need to remove columns of betas_cord for Sample Names not in bmi14
a <- t(bmi14)
colnames(a) <- a[1,]
a <- as.data.frame(a[-1, ])
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi14 <- bmi14[order(bmi14$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi14[c(3:24)]
cpg_assocF14 <- cpg.assoc(betas_cord, bmi14$bmi, covariates = covar)
cpg_assocF14
...
cpg_assoc_results = cpg_assocF14$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF14 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF14,cpg_locations$TargetID,cpg_locations$CHR,cpg_locations$MAPINFO)

```

```{r}
bmi_data_linked %>%
  filter(time == "F14"& mumsmoke!= -1) %>%
