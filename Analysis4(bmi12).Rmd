...
bmi12 <- bmi_data %>% filter(time == "F12") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi12 <- bmi12[order(bmi12$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi12[c(3:24)]
cpg_assocF12 <- cpg.assoc(betas_cord, bmi12$bmi, covariates = covar)
cpg_assocF12
...
cpg_assoc_results = cpg_assocF12$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF12 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF12,cpg_locations$TargetID,cpg_locations$CHR,cpg_locations$MAPINFO)
