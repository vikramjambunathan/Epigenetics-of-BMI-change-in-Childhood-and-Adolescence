...
bmi18 <- bmi_data %>% filter(time == "F18") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
```{r}
# For bmi18, we need to remove columns of betas_cord for Sample Names not in bmi18
a <- t(bmi18)
colnames(a) <- a[1,]
a <- as.data.frame(a[-1, ])
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi18 <- bmi18[order(bmi18$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi18[c(3:24)]
cpg_assocF18 <- cpg.assoc(betas_cord, bmi18$bmi, covariates = covar)
cpg_assocF18
...
cpg_assoc_results = cpg_assocF18$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF18 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF18,cpg_locations$TargetID,cpg_locations$CHR, cpg_locations$MAPINFO)

```

```{r}
bmi_data_linked %>%
  filter(time == "F18"& mumsmoke!= -1) %>%
