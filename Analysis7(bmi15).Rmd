...
# Considering bmi at 15 years.

bmi15 <- bmi_data %>% filter(time == "F15") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
```{r}
# For bmi15, we need to remove columns of betas_cord for Sample Names not in bmi15
a <- t(bmi15)
colnames(a) <- a[1,]
a <- as.data.frame(a[-1, ])
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi15 <- bmi15[order(bmi15$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi15[c(3:24)]
cpg_assocF15 <- cpg.assoc(betas_cord, bmi15$bmi, covariates = covar)
cpg_assocF15
...
cpg_assoc_results = cpg_assocF15$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF15 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF15,cpg_locations$TargetID,cpg_locations$CHR, cpg_locations$MAPINFO)

```

```{r}
bmi_data_linked %>%
  filter(time == "F15"& mumsmoke!= -1) %>%
