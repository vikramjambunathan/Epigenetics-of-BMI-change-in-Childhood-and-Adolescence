...
bmi11 <- bmi_data %>% filter(time == "F11") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
```{r}
# For bmi11, we need to remove columns of betas_cord for Sample Names not in bmi11
a <- t(bmi11)
colnames(a) <- a[1,]
a <- as.data.frame(a[-1, ])
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi11 <- bmi11[order(bmi11$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi11[c(3:24)]
cpg_assocF11 <- cpg.assoc(betas_cord, bmi11$bmi, covariates = covar)
cpg_assocF11
...
cpg_assoc_results = cpg_assocF11$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF11 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF11,cpg_locations$TargetID,cpg_locations$CHR, cpg_locations$MAPINFO)
