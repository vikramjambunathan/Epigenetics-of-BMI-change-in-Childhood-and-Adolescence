...
bmi_data %>%
  filter(age > 6) %>%
  ggplot(aes(x = time, y = bmi)) + geom_boxplot()
?reorder

#save(bmi_data, resultF7, resultsF9, resultsF11, file = "result.RData")
```
...
bmi24 <- bmi_data %>% filter(time == "F24") %>% select(Sample_Name,bmi,c(8:29)) %>% na.omit()
...
```{r}
# For bmi24, we need to remove columns of betas_cord for Sample Names not in bmi24
a <- t(bmi24)
colnames(a) <- a[1,]
a <- as.data.frame(a[-1, ])
...
betas_cord <- betas_cord %>% select(sort(names(.)))
bmi24 <- bmi24[order(bmi24$Sample_Name),]
...
# Running cpg.assoc
covar <- bmi24[c(3:24)]
cpg_assocF24 <- cpg.assoc(betas_cord, bmi24$bmi, covariates = covar)
cpg_assocF24
...
cpg_assoc_results = cpg_assocF24$results
cpg_locations = data.frame(Locations)
cpg_locations$CPG.Labels = row.names(cpg_locations)
resultF24 <- merge(cpg_assoc_results, cpg_locations)
```
...
manhattan(cpg_assocF24,cpg_locations$TargetID,cpg_locations$CHR, cpg_locations$MAPINFO)
